...
function tf_contactform_shortcode($atts){
    ...
    if($form_exists){
        ...
        $out .='<h3 id="header_message">'.urldecode($form['header_message']).'</h3><div id="form_messages" class="submit_message" ></div><div class="clear"></div>';
        ...
        $out.='<div class="comment-form"><form id="contactForm" action="" method="post" class="ajax_form contactForm" name="contactForm"><div class="row">';
        ...
        $out_left .= '</div></div>';
        $out_right .= '<div class="row rowSubmit">
        <input type="reset" value="'.urldecode($form['reset_button']).'" class="btn button_link_medium btn_gray text-semibold alignright" onclick="resetFields(this,event)">
        ...
        <input type="submit" id="send_form" class="btn-submit2 btn button_link_medium color btn_pink text-semibold alignright" name="submit" title="Submit mesage" value="'. $form['submit_mess'].'" />
...
function text($input,$form_input){
    if($form_input['classes']!=' omega ')$dist = 12;
        ...
        return "<div class='row field_text infieldlabel ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+$dist)."px; ".$form_input['floating']."'>
                <input type='text' style='width:".$form_input['ewidthpx']."px;' class='".$input['properties']['class']."' name='".TF_THEME_PREFIX.'_'. trim($form_input['shortcode'])."' value='".$form_input['label_text']."' onblur=' if (this.value == \"\") {this.value = \"".$form_input['label_text']."\";}' onfocus='if (this.value == \"".$form_input['label_text']."\") {this.value = \"\";}'  />
            </div>";
}

function textarea($input,$form_input){
    return "<div class='row field_textarea field_text infieldlabel' style='".$form_input['floating']."'>
                <textarea style='width:".$form_input['ewidthpx']."px;' class='".$input['properties']['class']."' name='".TF_THEME_PREFIX.'_'.trim($form_input['shortcode'])."' rows='10' value='".$form_input['label_text']."' onblur=' if (this.value == \"\") {this.value = \"".$form_input['label_text']."\";}' onfocus='if (this.value == \"".$form_input['label_text']."\") {this.value = \"\";}'></textarea>
            </div>";
}
...
function captcha($input,$form_input){
    ...
    $out="<div class='row field_text captcha ' style='".$form_input['floating']."'>
            <label class='label_title' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'>".$form_input['label_text']."</label><br>
