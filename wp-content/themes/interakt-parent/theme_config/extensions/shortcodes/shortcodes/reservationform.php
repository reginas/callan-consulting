...
function tf_reservationform_shortcode($atts){
    ...
    if($form_exists){
        $out .='<div class="add-comment" id="addcomments"><h3 id="header_message">'.urldecode($form['header_message']).'</h3>';
        ...
        $out.='<div class="comment-form"><form id="reservationForm" action="" method="post" class="reservationForm" name="reservationForm"><div class="row">';
        ...
        $fcount = 1;
        $margin=20;
        $out_left = '<div class="col col_1_2 alpha"><div class="inner">';
        $out_right = '<div class="col col_1_2 omega"><div class="inner">';
        foreach($colmuns as $form_input){
            ...
            {
                if($is_preview)
                    $ewidth=286-$lines[$element_line]+$margin;
                else
                    $ewidth=435-$lines[$element_line]+$margin;
            }
            else {
                $ewidth=286-$lines[$element_line]+$margin;
            }
        ...
        $out_left .= '</div></div>';
        $out_right .= '<div class="clear"></div><div class="row rowSubmit">
        <input type="reset" value="'.urldecode($form['reset_button']).'" class="reset_btn btn button_link_medium btn_gray text-semibold alignright" onclick="resetFields(this,event)">
        <a href="#" id="sending" class="btn button_link_medium color btn_pink text-semibold alignright" style="display: none;"><span>'.__('Sending ...','tfuse').'</span></a>
        <input type="submit" id="send_reservation" class="btn-submit2 btn button_link_medium color btn_pink text-semibold alignright" name="submit" title="Submit mesage" value="'. urldecode($form['submit_mess']).'" />
        <img id="sending_img" src="'.$surse.'" alt="Sending" style="margin-left:75px; margin-bottom:-23px; display: none; border:0;" /></div>
        </div></div>';
        $out .= $out_left.$out_right;
        $out .='</div></form><div class="clear"></div></div></div>';
    } else {
...
function res_text($input,$form_input){
    ...
    return "<div class='row field_text infieldlabel ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+$dist)."px; ".$form_input['floating']."'>
                <input type='text' style='width:".$form_input['ewidthpx']."px;' class='".$input['properties']['class']."' name='".TF_THEME_PREFIX.'_'. trim($form_input['shortcode'])."' value='".__($form_input['label_text'],'tfuse')."' onblur=' if (this.value == \"\") {this.value = \"".__($form_input['label_text'],'tfuse')."\";}' onfocus='if (this.value == \"".__($form_input['label_text'],'tfuse')."\") {this.value = \"\";}'/>
            </div>";
}
function res_textarea($input,$form_input){
    return "<div class='row field_textarea field_text infieldlabel".$form_input['classes']."' style='".$form_input['floating']."'>
                <textarea style='width:".$form_input['ewidthpx']."px !important;' class='".$input['properties']['class']."' name='".TF_THEME_PREFIX.'_'.trim($form_input['shortcode'])."' rows='10' value='".__($form_input['label_text'],'tfuse')."' onblur=' if (this.value == \"\") {this.value = \"".__($form_input['label_text'],'tfuse')."\";}' onfocus='if (this.value == \"".__($form_input['label_text'],'tfuse')."\") {this.value = \"\";}'></textarea>
            </div>";
}
function res_radio($input,$form_input){
    ...
    $output="<div class='row input_styled inlinelist ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+$dist)."px;".$form_input['floating']."'>
...
function res_datepicker($input,$form_input){
    ...
    $output="<div class='row field_text ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+$dist+$dist_select)."px; ".$form_input['floating']."'>
        <input style='width:".($form_input['ewidthpx']-25)."px;' type='text' class='".$input['properties']['class']."' name='".TF_THEME_PREFIX.'_'. trim($form_input['shortcode'])."' value='".__($form_input['label_text'],'tfuse')."' onblur=' if (this.value == \"\") {this.value = \"".__($form_input['label_text'],'tfuse')."\";}' onfocus='if (this.value == \"".__($form_input['label_text'],'tfuse')."\") {this.value = \"\";}'/></div>";
    return $output;
}
function res_checkbox($input,$form_input){
    ...
    $output = "<div class='row input_styled checklist ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+$dist)."px;".$form_input['floating']."'>
...
function res_select($input,$form_input){
    ...
    $out = "<div class='row field_select ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+$dist)."px; ".$form_input['floating']."'>
                <label class='label_title' for='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "'>".__($form_input['label_text'],'tfuse')."</label>
                <select style='width:".($form_input['ewidthpx']+12)."px;' class='".$input['properties']['class']."' name='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "' id='" .trim($input['id']). "'>";
    if(is_array($form_input['options'])){
        foreach ($form_input['options'] as $key => $option) {
