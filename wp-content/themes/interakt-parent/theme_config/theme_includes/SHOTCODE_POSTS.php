    ...
    function tfuse_shortcode_posts($args = null)
    {
        ...
        if( !empty($posts) )
        {
            ...
            foreach($posts as $post_elm)
            {
                ...
                $tf_post_option[$count]['post_loveit']   =  get_post_meta($post_elm->ID,'tfuse_love_it', true);
                $count++;
            }
            wp_reset_postdata();
        }
...
//show posts from many categories
if ( !function_exists('tfuse_shortcode_carousel_posts')):

    function tfuse_shortcode_carousel_posts($category)
    {
        global $post;
		
        $categories = explode(",",$category);
        
            foreach($categories as $cat_names):
                    $cat_ids[] = get_cat_ID($cat_names);
                    if($cat_ids == 0)break;
            endforeach;
        if($cat_ids[0] != 0)
        {
            $cat_ids = implode(",",$cat_ids);
            $args = array( 'numberposts' => -1,'orderby' => 'post_date', 'category' => $cat_ids);
        ...
        if( !empty($posts) )
        {
            foreach ($posts as $post_elm)
            {
                    ...
                    $img = '';
                            ...
                            {
                                    $get_image = new TF_GET_IMAGE();
                                    $img = $get_image->properties(array('class' => $image_class, 'alt' => get_the_title($post_elm->ID)))->width(100)->height(100)->src($post_image_src)->resize(true)->get_img();
                            }
...
//show posts from term
if ( !function_exists('tfuse_shortcode_custom_posts')):

    function tfuse_shortcode_custom_posts($taxonomy)
    {
        ...
        if( !empty($posts) )
        {
            ...
            foreach($posts as $post_elm)
            {
                ...
                $img = '';
                    ...
                    {
                        $get_image = new TF_GET_IMAGE();
                        $img = $get_image->properties(array('class' => $image_class, 'alt' => get_the_title($post_elm->ID)))->width(280)->height(170)->src($post_image_src)->resize(true)->get_img();
                    }
                ...
                $tf_post_option[$count]['post_link']            =  get_permalink($post_elm->ID);
                $tf_post_option[$count]['post_id']		= $post_elm->ID ;
                $tf_post_option[$count]['tags'] = wp_get_post_terms($post_elm->ID,'tags');
