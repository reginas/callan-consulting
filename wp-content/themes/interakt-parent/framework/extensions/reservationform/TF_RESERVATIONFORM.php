...
class TF_RESERVATIONFORM extends TF_TFUSE
{
    ...
    function list_reservations()
    {
        ...
        else
        {
            ...
            $only_for_counting = new WP_Query(array_merge($args, array('posts_per_page'    => -1)));
    ...
    function reservation_details()
    {
        ...
        foreach ($the_form['input'] as $key=>$input) {
            ...
            $option['type'] = 'raw';
            $option['html'] ='<div class="res_det_messages">'.((in_array($input['type'],array(7,8)))?date_i18n(get_option('date_format') ,strtotime($the_post[TF_THEME_PREFIX . '_' . $input['shortcode']])):urldecode($html) ).'</div>';
            $option['value'] = '';
            end($the_form['input']);
    ...
    function ajax_datepicker_row(){
        ...
        echo json_encode(array( 'html'=>$this->interface->cf_row_template($this->date_in_out(8,$values,$this->request->POST('number')))));die;
    }
    ...
    function submitFrontendForm()
    {
        ...
        if($status){
            ...
            if(isset($form['new_res_email_template']) && trim($form['new_res_email_template']) != '')
            {
                ...
                foreach($form['input'] as $input){
                    $value = (in_array($input['type'],array(7,8)))?date_i18n(get_option('date_format') ,strtotime($this->request->POST(TF_THEME_PREFIX . '_' . $input['shortcode']))):$this->request->POST(TF_THEME_PREFIX . '_' . $input['shortcode']);
                    $message .= '<strong>'. urldecode($input['label']) . ':</strong> '.$value. '<br />';
                }
    ...
    function save_form()
    {
        ...
        foreach ($form['tf_rf_input_options_label'] as $key => $value) {
            if ($form['tf_rf_select'][$key] == 2 || $form['tf_rf_select'][$key] == 4) {
        ...
        if($this->input->is_ajax_request()) {
            ...
            wp_update_term($form_id, 'reservations', array('description' => serialize($form_to_save)));
    ...
    function date_in_out($field,$values,$nr_in_form){
        ...
        $datepickers_types = array(7 => __('Check In', 'tfuse'), 8 => __('Check Out', 'tfuse'));
        $datepickers =  array(
