...
class TF_SEEK extends TF_TFUSE {
    ...
    public function get_seek_option($id, $default = NULL) {
        return $this->model->get_option($id, $default);
    }
    ...
    public function print_form_item($id){
        ...
        $template_vars = apply_filters('tf_ext_seek_print_item_template_vars',
            apply_filters('tf_ext_seek_print_item_template_vars__formid_'.$form_id,
                apply_filters('tf_ext_seek_print_item_template_vars__itemid_'.$id,
                    apply_filters('tf_ext_seek_print_item_template_vars__formid_'.$form_id.'_itemid_'.$id,
                        $template_vars
                    )
                )
            )
        ...
        do_action('tf_ext_seek_print_item_before');
        do_action('tf_ext_seek_print_item_before__formid_'.$form_id);
        do_action('tf_ext_seek_print_item_before__itemid_'.$id);
        do_action('tf_ext_seek_print_item_before__formid_'.$form_id.'_itemid_'.$id);

        if(trim($item['template'])){
            $this->load->ext_file($this->_the_class_name, '/views/items/' . $item['template'] . '.php', $template_vars);
        }

        do_action('tf_ext_seek_print_item_after');
        do_action('tf_ext_seek_print_item_after__formid_'.$form_id);
        do_action('tf_ext_seek_print_item_after__itemid_'.$id);
        do_action('tf_ext_seek_print_item_after__formid_'.$form_id.'_itemid_'.$id);
    }
