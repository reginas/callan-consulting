    ...
    if(typeof tf_optionize.one_time=='undefined') {
        ...
        (function(){
            ...
            $('.postbox').each(function(){
                $(this).on('click', '.multi_upload2_button', function()
                {
                    var $button = $(this);
                    var inputId = $button.attr('data-id');
                    var tab     = $button.attr('data-tab') || 'type' ;
                    var title;
                    var $hidden = $button.closest('.option').find('input[type="hidden"]:first');
                            ...
                            $("#TB_window").on('tb_unload'+ eventsNamespace, function(){
                                ...
                                $.ajax({
                                    ...
                                    success: function(r){
                                        ...
                                        var $wrapper = $button.closest('.multi_upload2_button_div');

                                        if (attachments.length) {
                                            $wrapper.find('.attachment_num').text(attachments.length);
                                            $button.text($wrapper.attr('data-text-multiple'));
                                            $button.attr('tab','gallery');
                                        } else {
                                            $wrapper.find('.attachment_num').text('0');
                                            $button.text($wrapper.attr('data-text-single'));
                                            $button.attr('tab', 'type');
                                        }
                                    },
                                    error: function(){
