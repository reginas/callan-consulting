...
class TF_INTERFACE extends TF_TFUSE
{
    ...
    public function admin_add_css()
    {
        ...
        if (tfuse_options('activate_tfuse_style', true)) {
    ...
    public function admin_add_js()
    {
        ...
        if ($this->request->isset_GET('page'))
        ...
        $this->include->js('TFE', 'framework_js', 'tf_head', 3, '1.0');
        ...
        if (method_exists($this->optigen, $row['type'])) {
            ...
            if(isset($row['name'])){
                if ($innerdocs != 'true')
                    ...
                    $title = ( empty($row['help']) ) ? $row['name'] : '<a rel="prettyPhoto[iframes]' . $row['id'] . '" title=' . __("Click for help", 'tfuse') . ' href="http://themefuse.com/pages/innerdocs/' . TF_THEME_PREFIX . '/help.php?page=' . $row['help'] . '&iframe=true&width=700&height=100%">' . $row['name'] . '</a>';
            }
