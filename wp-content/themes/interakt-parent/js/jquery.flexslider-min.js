...
(function (jQuery) {
  ...
  jQuery.flexslider = function(el, options) {
    var slider = jQuery(el),
        vars = jQuery.extend({}, jQuery.flexslider.defaults, options),
    ...
    methods = {
      init: function() {
        ...
        slider.slides = jQuery(vars.selector, slider);
        slider.container = jQuery(slider.containerSelector, slider);
        slider.count = slider.slides.length;
        // SYNC:
        slider.syncExists = jQuery(vars.sync).length > 0;
        // SLIDE:
        ...
        // CONTROLSCONTAINER:
        if (vars.controlsContainer !== "") slider.controlsContainer = jQuery(vars.controlsContainer).length > 0 && jQuery(vars.controlsContainer);
        // MANUAL:
        if (vars.manualControls !== "") slider.manualControls = jQuery(vars.manualControls).length > 0 && jQuery(vars.manualControls);
        ...
        // KEYBOARD:
        if (vars.keyboard && (jQuery(slider.containerSelector).length === 1 || vars.multipleKeyboard)) {
        ...
        if (!fade || (fade && vars.smoothHeight)) jQuery(window).bind("resize focus", methods.resize);
      ...
      asNav: {
        setup: function() {
          ...
          slider.slides.click(function(e){
            ...
            if (!jQuery(vars.asNavFor).data('flexslider').animating && !jQueryslide.hasClass('active')) {
      ...
      directionNav: {
        setup: function() {
          var directionNavScaffold = jQuery('<ul class="' + namespace + 'direction-nav"><li><a class="' + namespace + 'prev" href="#">' + vars.prevText + '</a></li><li><a class="' + namespace + 'next" href="#">' + vars.nextText + '</a></li></ul>');
          ...
          slider.directionNav.bind(eventType, function(event) {
            event.preventDefault();
            var target = (jQuery(this).hasClass(namespace + 'next')) ? slider.getTarget('next') : slider.getTarget('prev');
            slider.flexAnimate(target, vars.pauseOnAction);
          });
          // Prevent iOS click event bug
          if (touch) {
      ...
      sync: function(action) {
        var jQueryobj = jQuery(vars.sync).data("flexslider"),
            target = slider.animatingTo;
        
        switch (action) {
          case "animate": jQueryobj.flexAnimate(target, vars.pauseOnAction, false, true); break;
    ...
    slider.flexAnimate = function(target, pause, override, withSync, fromNav) {
      ...
      if (!slider.animating && (slider.canAdvance(target, fromNav) || override) && slider.is(":visible")) {
        if (asNav && withSync) {
          var master = jQuery(vars.asNavFor).data('flexslider');
